...
let typecheck lexbuf =
  ...
  let untypedTree = Parser.program Lexer.token lexbuf in
  Weed.weed untypedTree;
  let typedTree,_,_ = Type.typeAST untypedTree in
...
let compile lexbuf =
  ...
  let (typedTree,indexTable,globalVar) = Type.typeAST untypedTree in
  if !pptype
  then write Pprint.ptTree typedTree name ".pptype.go";
  write (Gen.generate indexTable globalVar) typedTree name ".wast";
...
let main =
  ...
  in
  let usage_msg = "Usage: <path_to_src>/main.native [lex|parse|weed|pretty|type|compile] [-dumpsymtab] [-smartsymtab] [-pptype] <path_to_programs>/foo/bar.go" in
...
*)
  ...
  begin
    ...
    if !file <> "" then
      ...
      begin
        if !version then
          print_endline "GoLite to WebAssembly compiler written in OCaml, version 4.0";
        if !help then
