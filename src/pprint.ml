...
(********************************************************************************)
(* print typed tree *)
(* let ptTree (Prog(pkg,decls)) outc = *)
let ptTree (tt:Ast.Typed.ast) outc =
  let tabc = ref 0 in (* tab count *)
  let pln() = Printf.fprintf outc "\n" in (* print line *)
  let pstr s = Printf.fprintf outc "%s" s in (* print ocaml string *)
  let rec tabWith n = if n <= 0 then () else (pstr "\t"; tabWith (n-1)) in
  ...
  let pcsl f = function (* print comma separated list *)
    ...
    | x::xs -> f x; List.iter (fun y -> pstr ", "; f y) xs
  in
  let rec pTyp t = match t with
    | TSimp(typ_id)  -> failwith "not done"
    | TStruct(x_typ_ls)  -> failwith "not done"
    | TArray(typ,d)  -> failwith "not done"
    | TSlice(typ)  -> failwith "not done"
    | Void  -> failwith "not done"
  in
  let rec pExpr (e:Ast.Typed.annotated_texpr) = match fst e with
    | Iden(i)  -> failwith "not done"
    | Parens(e)  -> failwith "not done"
    | AValue(e1,e2)  -> failwith "not done"
    | SValue(e,i)  -> failwith "not done"
    | ILit(d)  -> failwith "not done"
    | FLit(f)  -> failwith "not done"
    | RLit(c)  -> failwith "not done"
    | SLit(s)  -> failwith "not done"
    | Bexp(op,e1,e2)  -> failwith "not done"
    | Uexp(op,e)  -> failwith "not done"
    | Fn_call(fun_id, es)  -> failwith "not done"
    | Append(x, e)  -> failwith "not done"
  in
  let rec pStmt (s:Ast.Typed.annotated_utstmt) = match fst s with
    | Assign(xs, es)  -> failwith "not done"
    | Print(es)  -> failwith "not done"
    | Println(es)  -> failwith "not done"
    | If_stmt(po,e,ps,pso)  -> failwith "not done"
    | Block(stmts)  -> failwith "not done"
    | Switch_stmt(po, eo, ps)  -> failwith "not done"
    | Switch_clause(eso, ps)  -> failwith "not done"
    | For_stmt(po1, eo, po2, ps)  -> failwith "not done"
    | Var_stmt(ids_eso_typo_ls)  -> failwith "not done"
    | SDecl_stmt(ids, eso)  -> failwith "not done"
    | Type_stmt(id_typ_ls)  -> failwith "not done"
    | Expr_stmt e  -> failwith "not done"
    | Return(eo)  -> failwith "not done"
    | Break  -> failwith "not done"
    | Continue  -> failwith "not done"
    | Empty_stmt  -> failwith "not done"

  in
  let rec pDecl d =
    tab(); match d with
    | Var_decl(ids_eso_typo_ls)  -> failwith "not done"
    | Type_decl(typId_typ_ls)  -> failwith "not done"
    | Func_decl(fId, id_typ_ls, typ, ps)  -> failwith "not done"
  in
  (* pstr ("package "^pkg); pln(); pln(); List.iter pDecl decls *)
  ()
